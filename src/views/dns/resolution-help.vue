    ...
    <div>
        <div v-if="focus=='name'" class="type">
            ...
            <div class="detail mt-6">
                <div>
                    <span class="key cursor" @click="setValue('name','www')">www</span>
                    <span class="item">常见主机记录，将域名解析为 www.{{ domain }}</span>
                </div>
                <div>
                    <span class="key cursor" @click="setValue('name','@')">@</span>
                    <span class="item">直接解析主域名 {{ domain }}</span>
                </div>
                <div>
                    <span class="key cursor" @click="setValue('name','mail')">mail</span>
        ...
        <div v-if="focus=='value'" class="type">
            ...
            <div class="detail mt-6">
                <div v-if="type=='A'||!type">
                    <span class="item">您选择的是 A 记录，请在此填写服务器 IP 地址，如 1.1.1.1</span>
                </div>
                <div v-if="type=='AAAA'||!type">
                    <span class="item">您选择的是 AAAA 记录，请在此填写希望指向的 IPv6 地址，如 2001:db8::1</span>
                </div>
                <div v-if="type=='CNAME'||!type">
        ...
        <div v-if="focus=='ttl'" class="type">
            ...
            <div class="detail mt-6">
                <div>
                    <span class="key cursor" @click="setValue('ttl',600)">600</span>
                    <span class="item">一般默认值，如不了解请保留 600 秒即可</span>
                </div>
                <div>
                    <span class="key cursor" @click="setValue('ttl',3600)">3600</span>
                    <span class="item">当记录值较少变动时，建议选择 3600 秒，有利于提升解析速度</span>
                </div>
                <div>
                    <span class="key cursor" @click="setValue('ttl',60)">60</span>
...
.detail .key{color: rgb(var(--primary-6)); display:inline-block; width:110px; padding:3px 0;}
.detail .item{color: var(--color-text-3); padding:10px 0;}
</style>
