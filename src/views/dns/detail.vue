<template>
    <div class="padding-20 dns-page">
                    ...
                    <tr v-for="item in records" :key="item.id">
                        ...
                        <td>
                            <a-tooltip content="编辑">
                                <i class="opt-icon" @click="openRecordForm(item)"><icon-edit /></i>
                            </a-tooltip>
                            <a-popconfirm content="确认要删除该记录吗" @ok="deleteRecord(item)" position="lt" class="popconfirm-delete" type="warning" :ok-button-props="{ status: 'danger' }">
        ...
        <a-drawer :width="620" :visible="form.show" @ok="submitRecord" @cancel="form.show=false">
            <template #title>{{ form.id ? '编辑记录' : '添加记录' }}</template>
            <a-form :model="form" auto-label-width>
                ...
                <a-form-item label="记录值" required>
                    <a-input v-model="form.value" :placeholder="valuePlaceholder" allow-clear />
                </a-form-item>
                <a-form-item v-if="form.type === 'MX'" label="MX优先级">
                    <a-input-number v-model="form.mxPriority" :min="1" :max="65535" />
                </a-form-item>
                <a-form-item label="TTL">
                    <a-input-number v-model="form.ttl" :min="1" :max="86400" />
                </a-form-item>
            </a-form>
        </a-drawer>
    </div>
