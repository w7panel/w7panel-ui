    ...
    <div class="padding-20 df df-c" style="height:100%;">
        ...
        <a-layout class="fc">
            <a-layout-sider :width="160">
                <a-menu v-if="menus.length" v-model:selected-keys="selectMenu" style="height:100%;width:160px;" @menu-item-click="changeKey">
...
export default {
    ...
    created(){
        ...
        bus.$on('createApp', this.createApp);
        bus.$on('editApp', this.editApp);
        bus.$on('deleteApp', this.deleteApp);
        bus.$on('getPodList', this.sendPodList);
        bus.$on('changeMenu', this.changeMenu);
        bus.$on('changeReplicas', this.changeReplicas);
    },
    unmounted(){
        bus.$off('createApp', this.createApp);
        bus.$off('editApp', this.editApp);
        bus.$off('deleteApp', this.deleteApp);
        bus.$off('getPodList', this.sendPodList);
        bus.$off('changeMenu', this.changeMenu);
        bus.$off('changeReplicas', this.changeReplicas);
    },
    mounted(){},
    methods: {
        ...
        init(){
            k8sproxy.get('/apis/microapp.w7.cc/v1alpha1/namespaces/'+this.namespaceActive+'/microapps?labelSelector=w7.cc/identifie=gpustack-backend&limit=500').then(res=>{
                if(!res?.data?.items?.[0]){
                ...
                this.app = item;
                let menus = item?.spec?.bindings?.[0]?.menu || [];
                menus.sort((a,b)=>b.displayorder-a.displayorder);
                
                this.menuActive = menus?.find(i=>i.is_default==2)?.do || this.menus?.[0]?.do || '';
                this.selectMenu = [decodeURIComponent(this.$route.query?.gpustackbox || '')?.replace(/^\//,'')];
...
// frontendUrl: 'http://localhost:8001',
        ...
        changeKey(v){
            this.menuActive = v;
// this.$router.push('/app/gpustack?gpustackbox='+encodeURIComponent(this.info.frontendUrl + this.menuActive)).then(res=>{
//     this.wujieInit();
// })
